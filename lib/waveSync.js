...
async function pushInvoiceToWave(invoiceId) {
  ...
  if (!invoice) return;
  if (invoice.waveInvoiceId && invoice.waveSyncedAt) {
    // Already fully synced — the only thing worth doing on a repeat call is
    // backfilling waveViewUrl for an invoice that synced before that field
    // existed. Looked up directly by its existing Wave id, never recreated.
    if (!invoice.waveViewUrl) {
      try {
        const viewUrl = await wave.getInvoiceViewUrl(invoice.waveInvoiceId);
        if (viewUrl) store.update('invoices', invoice.id, { waveViewUrl: viewUrl });
      } catch (err) {
        log(`Failed to backfill Wave view url for invoice #${invoiceId}`, err);
      }
    }
    return;
  }
