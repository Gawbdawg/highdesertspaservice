...
// unitPrice is a required field on Wave's ProductCreateInput, but it's not
// actually used by anything here — every invoice line item explicitly sets its
// own unitPrice when it's added to an invoice (see buildWaveLineItems in
// lib/waveSync.js), which overrides whatever the product's own default is. This
// product only exists as a named anchor for the line item, so 0 is a safe,
// inert default.
async function createProduct({ name }) {
  const data = await request(
    ...
    { input: { businessId: businessId(), name, unitPrice: 0, incomeAccountId: null } },
    'productCreate',
  );
