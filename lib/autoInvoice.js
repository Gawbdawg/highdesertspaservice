...
function resolvePrice(service, customerId) {
  ...
  // Home/owner custom pricing is a negotiated rate for the regular recurring "clean"
  // (the service priced by visit frequency) — it should never leak onto a flat-priced
  // catalog entry like "Diagnostic Visit" or a one-off repair charge just because a
  // custom-pricing map happens to have a value stored under that service's id (e.g.
  // left over from before this filter existed). Flat-priced services always bill at
  // their own listed price.
