...
// True if `date` falls anywhere inside a "block" booking on this property — an
// owner-declared stretch where the property is fully unavailable for service (e.g.
// a renovation, painting, or anything else where nobody should be scheduled to visit
// at all), as opposed to a normal guest stay (see guestPresentOn above). Unlike
// guestPresentOn, this is an INCLUSIVE range on both ends: a guest booking treats its
// checkout day as still fair game for a same-day turnover cleaning (that's the whole
// point of the strict `<` there), but a block has no such exception — the property is
// off-limits for its full stated range, first day through last.
function propertyBlockedOn(customerId, date) {
  return store.getAll('bookings').some(
    (b) => b.customerId === customerId && b.type === 'block' && b.startDate <= date && date <= b.endDate
  );
}
...
// guests) — and skips entirely if the property is under an owner-declared block that
// day (see propertyBlockedOn).
//
...
function maybeCreateCheckoutAppointment(customerId, checkoutDate) {
  ...
  if (guestPresentOn(customerId, checkoutDate) || propertyBlockedOn(customerId, checkoutDate)) return null;
...
// Called the moment an owner adds a "block" booking (property unavailable, e.g. a
// renovation) — cancels EVERY still-'scheduled' visit that falls inside the block's
// full inclusive range, no matter how it was scheduled or by whom. This is
// deliberately broader than cancelConflictingCheckoutAppointments below (which only
// ever touches its own previously auto-scheduled turnover cleanings, and only when a
// guest booking reveals a real conflict) — a block is an explicit, unambiguous
// declaration that nothing should happen on this property during this window, so any
// visit in range gets cancelled, not just ones this feature created itself.
function cancelAppointmentsInBlock(customerId, startDate, endDate, blockNotes) {
  ...
  appts.forEach((a) => {
    store.update('appointments', a.id, {
      status: 'cancelled',
      notes: `Auto-cancelled: property blocked${blockNotes ? ` (${blockNotes})` : ''} from ${startDate} to ${endDate}.`,
    });
    cancelledCount += 1;
  });
