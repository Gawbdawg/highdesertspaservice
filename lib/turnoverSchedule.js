...
function maybeCreateCheckoutAppointment(customerId, checkoutDate) {
  ...
  // A cancelled appointment on this date only counts as "already handled" if a human
  // (or an active business rule) meant for it to stay cancelled. It doesn't if the
  // cancellation came from the old "block dates skip service" feature — that feature
  // was fully reverted (all block dates get serviced again), so a leftover
  // cancellation it left behind should no longer block a real checkout from getting
  // rescheduled. Every other cancellation reason (guest still present, owner-cancelled
  // visit, etc.) still blocks re-creation, same as before.
  const already = store.getAll('appointments').some(
    (a) => a.customerId === customerId
      && (a.date === checkoutDate || a.checkoutDate === checkoutDate)
      && !(a.status === 'cancelled' && (a.notes || '').startsWith('Auto-cancelled: property blocked'))
  );
