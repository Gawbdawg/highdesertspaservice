...
function bundleDraftInvoices(customerIds, monthStr, invoiceFields) {
  ...
  const combined = store.create('invoices', {
    ...
    appointmentId: null,
    amount: total,
    // The 1st of the billing month, not "whenever the admin happened to click
    // Generate" — a monthly invoice for August should read August 1st whether it
    // actually got generated on the 1st or the 15th.
    issuedDate: `${monthStr}-01`,
    dueDate: '',
    status: 'draft',
    lineItems,
  });
